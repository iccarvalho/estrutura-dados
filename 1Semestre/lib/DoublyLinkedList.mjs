...
export default class DoublyLinkedList {
    ...
    // Método que retorna a posição do no cujo conteúdo foi especificado
    indexOf(value){
        ...
        for(let i = 0; i < middle; i++){
            // verifica se o valor está no head
            if(value === node1.data) return i;

            // verifica se o valor está no tail
            if(value === node2.data) return this.#count - 1 - i;

            // node1 avança via next
            node1 = node1.next;

            // node2 recua via prev
            node2 = node2.prev;
        }
